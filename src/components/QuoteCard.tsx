...
const QuoteCard = ({ selectedSlot }: QuoteCardProps) => {
    ...
    const effectiveAppt: Slot | null = selectedSlot ?? (data.appointment ?? null);
    ...
    const quoteNumber = useMemo(() => {
        const d = new Date();
        return `DV-${d.getFullYear()}${String(d.getMonth() + 1).padStart(2, "0")}${String(
            d.getDate()
                    ...
                    <CardContent sx={{ p: 3 }}>
                        ...
                        <Stack spacing={1.2} mb={2}>
                            ...
                            <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1 }}>
                                ...
                                {effectiveAppt && (
                                    <Chip
                                        label={`RDV: ${new Date(effectiveAppt.start).toLocaleString("fr-FR", {
                                            dateStyle: "medium",
                                            timeStyle: "short"
                                        })}`}
                                        color="success"
